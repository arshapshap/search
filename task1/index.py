...
def is_cyrillic(text):
    return len([x for x in text.lower() if 'а' <= x <= 'я' or x == 'ё']) > len(text) / 2
...
def crawl_pages(urls, output_dir, docs_number, words_min_number):
    ...
    with open('index.txt', 'w', encoding='utf-8') as index_file:
        while queue and doc_counter < docs_number:
            url = queue.popleft()
            ...
            if not is_cyrillic(text_content) or word_count < words_min_number:
                continue
            ...
            index_file.flush()
            print(f'Saved: {filename} ({word_count} words)\t-- {decoded_url}')
...
if __name__ == '__main__':
    ...
    crawl_pages(urls=args.urls, output_dir='docs', docs_number=100, words_min_number=1000)
